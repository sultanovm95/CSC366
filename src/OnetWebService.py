...
class OnetWebService:
    def __init__(self, username, password):
        self._headers = {
            ...
            "Authorization": "Basic "
            + base64.standard_b64encode((username + ":" + password).encode()).decode(),
    ...
    def call(self, path, *query):
        ...
        if len(query) > 0:
            url += "?" + parse.urlencode(query, True)
        ...
        try:
            ...
            if e.code == 422:
                ...
                return {
                    "error": "Call to " + url + " failed with error code " + str(e.code)
                }
        ...
        if (code != 200) and (code != 422):
            return {
                "error": "Call to " + url + " failed with error code " + str(code),
                "urllib2_info": handle,
            }
